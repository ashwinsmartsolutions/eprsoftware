        ...
        {productionHistory.length === 0 ? (
          ...
          <div className="space-y-3">
            {productionHistory.map((record, index) => {
              const total = record.totalProduced || Object.values(record.stock).reduce((a, b) => a + b, 0);
              return (
                <div
                  key={record._id}
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500 font-medium">bottles</p>
                    </div>
                  </div>
                  {/* Flavor Details */}
                      ...
                      {flavors.map((f) => (
                        <div key={f.key} className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-2 text-center border border-gray-100">
                          ...
                          <p className="text-base font-bold text-gray-900">
                            {record.stock[f.key]?.toLocaleString() || '0'}
                          </p>
                        </div>
                      ))}
                    </div>
